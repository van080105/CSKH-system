    ...
    <div className="relative" ref={menuRef}>
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-56 bg-white shadow-lg rounded-lg py-2 z-50 divide-y divide-neutral-200">
          <MenuItem icon="src/assets/profile.svg" label="Quản lý thông tin" />
          <MenuItem icon="src/assets/key.svg" label="Đổi mật khẩu" />
          <MenuItem icon="src/assets/audit.svg" label="Nhật ký hoạt động" />
          <MenuItem icon="src/assets/logout.svg" label="Đăng xuất" danger />
        </div>
      )}
    </div>
  );
...
const MenuItem = ({ icon, label, danger }) => (
  <button
    ...
    `}
  >
    {/* Kiểm tra nếu icon là đường dẫn ảnh */}
