...
export default function Contact() {
  ...
  useEffect(() => {
    ...
    if (message.toLowerCase().includes("đăng nhập"))
      setHint("💡 Bạn có thể thử đặt lại mật khẩu nếu quên. Hãy cho chúng tôi biết email đăng ký nhé!")
    else if (message.toLowerCase().includes("thanh toán"))
      setHint("💳 Vui lòng cung cấp mã đơn hàng hoặc phương thức thanh toán bạn đang dùng.")
    else if (message.length > 80)
      setHint("✍️ Cảm ơn bạn đã mô tả chi tiết! Chúng tôi sẽ xem xét kỹ để hỗ trợ tốt nhất.")
    else setHint("")
  }, [formData.message])
          ...
          {status === "success" ? (
            ...
            >
              {[
                ...
                { label: "Tiêu đề", name: "title", type: "text", placeholder: "Ví dụ: Cần hỗ trợ đăng nhập" },
              ].map((f, i) => (
                <motion.div
                  key={f.name}
